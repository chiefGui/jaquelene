...
The desktop adapter supplies an independently stored AI action model, with no campaign fallback. Its renderer-scoped session bounds concurrent operations and cancels work on explicit cancellation, document navigation, renderer loss, and window closure. Closing waits for active operations to release their resources; cancellation does not hide cleanup failures from diagnostics. Model resolution and inference each have a two-minute deadline. Provider attempts settle on success, failure, and interruption; successful provider usage remains recorded even when result validation rejects the text.

Editor results replace only the unsaved field value. The affected field is read-only while running, and saving waits until the action settles. Left/right controls beside Preview browse temporary text versions, retaining manual edits to each version. Save commits the selected text; Cancel restores the saved title and text. Both discard temporary versions and keyboard edit history. The renderer's pure `TextVersions` state, `useTextVersions` adapter, and `TextVersionControls` are independent of AI, Markdown, and persistence. Forms compose these pieces and own the save/cancel boundary; Markdown supplies a toolbar slot and keyboard-history reset. No action-result table, conversation, or automatic prompt save is created. No database reset is required for editor AI actions.
