...
It owns SQLite, migrations, IDs, scenarios, campaigns, threads, durable turn submission and retry, reply preparation, generation state, active generation supervision, the provider registry, and storage measurement. Platform code supplies provider adapters and filesystem locations, then consumes the plain TypeScript facade returned by `createBackend`.
...
Threads remain independent of campaigns. The turn service composes a thread write, semantic model-input preparation, and generation into the durable submit/retry workflow; callers supply only a thread identity and model.
