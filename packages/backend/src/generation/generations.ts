...
export function createGenerations({
  ...
  async function executeAcceptedReply(
    { generation, anchor, activeMessageId, regenerationSourceContent }: AcceptedReplyGeneration,
    signal?: AbortSignal,
    ...
    try {
      ...
      if (generation.regeneration !== undefined) {
        if (regenerationSourceContent === undefined) {
          throw new Error("Guided regeneration requires its accepted source response.");
        }
