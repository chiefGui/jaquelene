...
Composer skills declare a `history` request in their backend definition. Shared execution resolves it and guards against changes to the selected messages, active head, or scenario before delivering a result. Each skill owns its instructions and requirements for usable context. History options are not renderer metadata.
