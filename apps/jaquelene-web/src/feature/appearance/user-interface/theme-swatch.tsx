...
const styles = stylex.create({
  root: {
    ...
    backgroundImage: `radial-gradient(circle at 72% 78%, color-mix(in oklch, ${colors.interactive} 24%, transparent), transparent 56%), linear-gradient(145deg, color-mix(in oklch, ${colors.backgroundRaised} 88%, transparent), color-mix(in oklch, ${colors.backgroundCanvas} 94%, transparent) 78%)`,
    ...
    width: "3.25rem",
    "::before": {
      backgroundImage: `radial-gradient(circle at 28% 24%, color-mix(in oklch, ${colors.foregroundPrimary} 42%, transparent) 0 5%, transparent 21%), linear-gradient(122deg, ${colors.reasoning}, ${colors.interactive} 48%, ${colors.storageAppData})`,
      borderRadius: "48% 52% 44% 56% / 34% 40% 60% 66%",
      bottom: "-18%",
      boxShadow: `inset 0 0.375rem 0.75rem color-mix(in oklch, ${colors.foregroundPrimary} 18%, transparent), 0 -0.1875rem 0.75rem color-mix(in oklch, ${colors.interactive} 34%, transparent)`,
      content: '""',
      height: "76%",
      left: "-18%",
      position: "absolute",
  ...
  selected: {
    outlineColor: colors.interactive,
  },
});
