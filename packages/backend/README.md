...
Editor results replace only the unsaved field value. The affected field is read-only while running, and saving waits until the action settles. Left/right controls browse temporary text versions, retaining manual edits to each version. Save commits the selected text; Cancel restores the saved title and text. Both discard temporary versions and keyboard edit history. No action-result table, conversation, or automatic prompt save is created. No database reset is required for editor AI actions.
