...
describe("regeneration instructions", () => {
  ...
  it("enforces the same UTF-16 bound for ASCII and supplementary characters", () => {
    ...
    expect(parseRegenerationInstructions("😀".repeat(1_000))).toHaveLength(2_000);
    expect(() => parseRegenerationInstructions("a".repeat(2_001))).toThrow("at most 2,000");
    expect(() => parseRegenerationInstructions("😀".repeat(1_001))).toThrow("at most 2,000");
    expect(() => parseRegenerationInstructions(null)).toThrow("at most 2,000");
  });
});
