...
describe("turns", () => {
  ...
  it("sends guidance only for its generation and persists it across reopening the database", async () => {
    ...
    const { databasePath, threads, turns } = await openTurnEnvironment(generate);
    const thread = threads.create();
    const configuration = { model: { providerId: "provider-a", modelId: "maker/model" } };
    const submission = await Effect.runPromise(
      turns.submit({ threadId: thread.id, content: "Hello", configuration }),
    ...
    const guided = await Effect.runPromise(
      turns.regenerate({
        assistantMessageId: original.assistantMessage.id,
        configuration,
    ...
    const completed = await Effect.runPromise(guided.settlement);
    ...
    const fresh = await Effect.runPromise(blank.settlement);
  ...
  it("rejects oversized guidance before accepting or calling the provider", async () => {
    ...
    const { threads, turns, database } = await openTurnEnvironment(generate);
    const thread = threads.create();
    const configuration = { model: { providerId: "provider-a", modelId: "maker/model" } };
    const submission = await Effect.runPromise(
      turns.submit({ threadId: thread.id, content: "Hello", configuration }),
  ...
  it("retains guidance when an accepted generation is recovered as interrupted", async () => {
    const generate = vi.fn<TestGenerate>().mockResolvedValue({ text: "Original reply" });
    const { database, generationEngine, threads, turns } = await openTurnEnvironment(generate);
    const thread = threads.create();
    const configuration = { model: { providerId: "provider-a", modelId: "maker/model" } };
    const submitted = await Effect.runPromise(
      turns.submit({ threadId: thread.id, content: "Hello", configuration }),
    );
    const original = await Effect.runPromise(submitted.settlement);
    if (original.outcome !== "completed") {
      throw new Error("Expected the original reply.");
    }
